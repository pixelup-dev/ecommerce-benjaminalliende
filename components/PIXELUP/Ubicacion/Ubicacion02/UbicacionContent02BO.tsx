...
export default function UbicacionContentBO() {
  ...
  const [content, setContent] = useState<any>({
    ...
    contentText: JSON.stringify({
      title: "Pixel Up",
      contentText:
        "<p>Pixel Up es una plataforma que te da acceso a una serie de herramientas.</p>",
      additionalData: {
  ...
  const fetchContent = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL_BO_CLIENTE}/api/v1/content-blocks/${process.env.NEXT_PUBLIC_UBICACION02_CONTENTBLOCK}?siteId=${process.env.NEXT_PUBLIC_API_URL_SITEID}`,
        {
          headers: {
  ...
  const handleInputChange = (e: InputEvent, field: keyof any) => {
    setFormData({ ...formData, [field]: e.target.value });
  };

  const handleNestedChange = (
    e: InputEvent,
    parent: keyof any["additionalData"],
    field?: string
  ) => {
