...
const Hero17ImagesBO: React.FC = () => {
  ...
  const fetchImages = async () => {
    try {
      ...
      const bannerId = `${process.env.NEXT_PUBLIC_HERO17_IMGID}`;
  ...
  const handleAddImageClick = (e: React.MouseEvent) => {
    ...
    if (bannerData.length >= 2 && !isAddingImage) {
      alert("Solo se permiten un máximo de 2 imágenes");
      return;
    }
  ...
  return (
    <div className="max-w-5xl mx-auto mt-12">
      <h3 className="text-lg font-semibold mb-4">Galería de Imágenes Hero 17</h3>
      <div className="text-sm text-gray-600 mb-4">
        Imágenes cargadas: {bannerData.length} de 2
      </div>
