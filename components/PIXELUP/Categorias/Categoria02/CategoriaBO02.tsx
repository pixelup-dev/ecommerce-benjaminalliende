...
const CategoriaBO02 = () => {
  ...
  const SliderSubmit = async (e: React.FormEvent) => {
    ...
    // Verificar si hay menos de 3 sliders antes de agregar uno nuevo
    if (slidersData.length >= 3) {
      toast.error("No se pueden agregar más de 3 sliders");
      return;
    }
    ...
    <section className="py-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        {slidersData.map((banner) => (
          <div
            key={banner.id}
            className="group cursor-pointer relative h-[400px] md:h-[500px] overflow-hidden rounded-lg"
          >
            <img
...
{/*                 <p className="text-gray-200 mb-2 text-base">
                ...
                <div className="bg-white text-gray-800 px-8 py-3 rounded font-bold transition-all duration-300 hover:bg-[#eea83b] inline-block">
                  Explorar Categoría
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            {getAvailableOrderNumbers().length === 0 && (
              <p className="text-sm text-red-500 mt-1">
                Has alcanzado el límite máximo de categorías (3)
              </p>
            )}
          </div>
