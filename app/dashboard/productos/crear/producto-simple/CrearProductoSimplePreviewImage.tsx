...
const CrearProductoSimple: React.FC = ({}) => {
  ...
  const handleSubmit = async (
    ...
    // Validar imagen preview
    if (!previewImage) {
      toast.error("Debes seleccionar una imagen preview para el producto");
      setIsLoading(false);
          ...
          if (pendingImageChanges.pendingImages.length > 0) {
            await Promise.all(
              pendingImageChanges.pendingImages.map((newImage) => {
                // Calcular content-length basado en el tamaño de los datos
